  ...
  class KinematicSolverInfo : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_link_namesi = (uint32_t *)(outbuffer + offset);
      *length_link_namesi = strlen( (const char*) this->link_names[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < link_names_length; i++){
      uint32_t length_st_link_names = *(uint32_t *)(inbuffer + offset);
      offset += 4;
