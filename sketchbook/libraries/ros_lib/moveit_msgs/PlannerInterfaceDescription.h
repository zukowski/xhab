  ...
  class PlannerInterfaceDescription : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_planner_idsi = (uint32_t *)(outbuffer + offset);
      *length_planner_idsi = strlen( (const char*) this->planner_ids[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < planner_ids_length; i++){
      uint32_t length_st_planner_ids = *(uint32_t *)(inbuffer + offset);
      offset += 4;
