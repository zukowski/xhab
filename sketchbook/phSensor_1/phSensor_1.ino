...
The TX1 line goes to the RX pin of your product.
The RX1 line goes to the TX pin of your product.
Make sure you also connect to power and GND pins to power and a common ground.
Open TOOLS > serial monitor, set the serial monitor to the correct serial port and set the baud rate to 18400.
...
boolean sensor_stringcomplete = false;                                         //have we received all the data from the Atlas Scientific product
  ...
  void setup(){                                                                //set up the hardware
     Serial.begin(9600);                                                      //set baud rate for the hardware serial port_0 to 18400
     Serial1.begin(9600);                                                     //set baud rate for software serial port_1 to 18400
     inputstring.reserve(5);                                                   //set aside some bytes for receiving data from the PC
     sensorstring.reserve(10);                                                 //set aside some bytes for receiving data from Atlas Scientific product
     }
