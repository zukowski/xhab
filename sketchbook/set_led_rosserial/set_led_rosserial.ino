...
void dcmotor_command(const xhab::DcMotor& dc_motor_msg){
  ...
  switch(dc_motor_msg.mode){
    case 0:
      analogWrite(DC_MOTOR_PWM,1);
    case 1:
      analogWrite(DC_MOTOR_PWM,126);
      delay(5000);
      analogWrite(DC_MOTOR_PWM,1);
    default:
      analogWrite(DC_MOTOR_PWM,1);
...
void setup() {
  Serial.begin(38400); //set baudrate for hardware
  Serial1.begin(38400); //ser baudrate for port 1
  for(int i=LED_1; i<LED_4+2; i+=2) {
