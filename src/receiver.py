...
def callback(data): 
     #Read in wrist position
     wrist_angle = wrist.read_angle();
     #Move each of the arm joints to recieved angle
     shoulder1.move_angle( math.radians( float( data.shoulder1_angle ) ) , blocking = False )
     shoulder2.move_angle( math.radians( float( data.shoulder2_angle ) ) , blocking = False )
     ...
     wrist.move_angle( math.radians( wrist_angle + float( data.wrist_angle ) ) , blocking = True )
