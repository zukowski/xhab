...
ros::Publisher callbak_data("/data/callback", &callback_msg);
...
void getArduinoState(int *stateArray){
  ...
  if(checkWaterStation == 1 && stateArray[6] == 1){
    waterStationState = 0;
  }
  else if(checkWaterStation == 1 && stateArray[6] == 0){
    waterStationState = 1;
  }
  else
    waterStationState = 0;
...
void setup() {  
  ...
  //start publisher for callback messages
...
void loop() {
  ...
  getArduinoStates(&arduinoStateArray);
